$(function(){
    ...
    var type,platType,Independence,partner;
    if((href.indexOf('/landi.html') > -1) || (href.indexOf('/landiCutform.html') > -1)){
        type = 0;
        platType = 3;
        partner = 3;
        _getcourseData(type,platType);
    }else if((href.indexOf('/likeshuo.html') > -1) || (href.indexOf('/lksCutform.html') > -1)){
        type = 0;
        platType = 2;
        partner = 2;
    ...
    if((href.indexOf('/landiCutform.html') > -1) || (href.indexOf('/lksCutform.html') > -1)){
        Independence = 2;
    }else{
        Independence = 1;
    }
	...
	$('.course_table tbody').on('click','.goodsBtn',function(){
		...
		$('#courseNum').val('');
        $.ajax({
        	url : apiUrl + "/openapi/pbstalkorder/addOrder/"+uniqId+".json",
            dataType: 'json',
        	type : 'POST',
        	success : function(data){
        		if(data.success == true){
